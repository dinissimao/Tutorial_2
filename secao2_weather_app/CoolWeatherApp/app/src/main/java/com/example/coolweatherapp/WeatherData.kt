...
// Dados horários (pressão, temperatura, código do tempo)
data class Hourly(
...
// Enumeração dos códigos WMO com o nome do drawable correspondente
// O sufixo "day" ou "night" é adicionado dinamicamente em runtime
// conforme o horário (dia ou noite) na localização consultada
...
// Função auxiliar que cria um mapa de código → enum
// Permite pesquisar rapidamente o enum pelo código devolvido pela API
fun getWeatherCodeMap(): Map<Int, WMO_WeatherCode> {
    val mapaMeteo = HashMap<Int, WMO_WeatherCode>()
    WMO_WeatherCode.values().forEach { entrada ->
        mapaMeteo[entrada.code] = entrada
    }
    return mapaMeteo
}